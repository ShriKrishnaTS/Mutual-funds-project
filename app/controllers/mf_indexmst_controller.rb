class MfIndexmstController < ApplicationController

  def read_mfindex_mstr
  ...
  def mfindex_master
    ...
    begin
      afile = File.open("app/assets/files/index_mst.ace","r")
      if afile
        ...
        afile.each { |line|
          ...
          if line == '<<eof>>'
            ...
            @result = 1
            insert_mfidex_mastr(@final_split)
            if (@result == 0)
  ...
  def insert_mfidex_mastr(insertdata)
    begin
      lc = LogcreatorController.new
      mf = IndexMst.new
	  select_sql = "select * from index_msts where indexcode = "+insertdata[0] 
	  @data = ActiveRecord::Base.connection.select_all(select_sql)
	  if(@data.length > 0)
		puts "Inside Delete loop"
		delete_sql = "Delete from index_msts where indexcode = "+insertdata[0]
		@del_data = ActiveRecord::Base.connection.delete(delete_sql)
	  end
      mf.indexcode = insertdata[0]
      mf.fincode = insertdata[1]
      mf.scripcode = insertdata[2]
      mf.indexname = insertdata[3]
      mf.upd_flag = insertdata[4]
      mf.save
      @result = 0
    rescue =>err
      puts  err.message
