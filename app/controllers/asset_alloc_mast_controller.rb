class AssetAllocMastController < ApplicationController

  def read_asset_alloc_mast
  ...
  def asset_alloc_master
  
	@result = 1
    @logfile = "asect_mst_new"
    @insert_count = 0
    @uninsert_count = 0
    @total_records = 0
    ...
    begin
      afile = File.open("app/assets/files/asect_mst_new.ace","r")
      if afile
  ...
  def asset_alloc_mast(insertdata)
    begin
      lc = LogcreatorController.new
      aam = AsectMstNew.new
	  select_sql = "select * from asect_mst_news where asect_code = "+insertdata[0] 
	  @data = ActiveRecord::Base.connection.select_all(select_sql)
	  if(@data.length > 0)
		puts "Inside Delete loop"
		delete_sql = "Delete from asect_mst_news where asect_code = "+insertdata[0]
		@del_data = ActiveRecord::Base.connection.delete(delete_sql)
	  end
      aam.asect_code = insertdata[0]
      aam.asect_type = insertdata[1]
      aam.asset = insertdata[2]
      aam.upd_flag = insertdata[3]
      aam.save
      @result = 0
    rescue =>err
      puts  err.message
