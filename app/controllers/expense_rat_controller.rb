class ExpenseRatController < ApplicationController

  def read_expense_ratio
  ...
  def expense_ratio
    ...
    begin
      afile = File.open("app/assets/files/expenceratio.ace","r")
      if afile
        ...
        afile.each { |line|
          ...
          if line == '<<eof>>'
            ...
            @result = 1
            insert_expense_ratio(@final_split)
            if (@result == 0)
  ...
  def insert_expense_ratio(insertdata)
    begin
      lc = LogcreatorController.new
      er = Expenceratio.new
      er.amc_code = insertdata[0]
      er.schemecode = insertdata[1]
      if insertdata[2].blank?
        er.date = DateTime.strptime('0001-01-01 00:00:00',"%Y-%m-%d %H:%M:%S").to_formatted_s(:db)
      else
        er.date = DateTime.strptime(insertdata[2],"%m/%d/%Y %I:%M:%S %p").to_formatted_s(:db)
      end

      er.exratio = insertdata[3]
      er.upd_flag = insertdata[4]
      er.save
      @result = 0
    rescue =>err
      puts  err.message
