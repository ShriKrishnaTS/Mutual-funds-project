class SectMstController < ApplicationController

  def read_sector_mast
  ...
  def sector_master
    ...
    begin
      afile = File.open("app/assets/files/sect_mst.ace","r")
      if afile
        ...
        afile.each { |line|
          ...
          if line == '<<eof>>'
            ...
            @result = 1
            insert_sector_mast(@final_split)
            if (@result == 0)
  ...
  def insert_sector_mast(insertdata)
    begin
      lc = LogcreatorController.new
      sem = SectMst.new
	  select_sql = "select * from sect_msts where sect_code = "+insertdata[0] 
	  @data = ActiveRecord::Base.connection.select_all(select_sql)
	  if(@data.length > 0)
		puts "Inside Delete loop"
		delete_sql = "Delete from sect_msts where sect_code = "+insertdata[0]
		@del_data = ActiveRecord::Base.connection.delete(delete_sql)
	  end
      sem.sect_code = insertdata[0]
      sem.sect_name = insertdata[1]
      sem.upd_flag = insertdata[2]
      sem.save
      @result = 0
    rescue =>err
      puts  err.message
