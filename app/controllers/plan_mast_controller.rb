class PlanMastController < ApplicationController

  def read_plan_master
   Thread.new do
			plan_masters()
	end
  end
  ...
  def plan_masters
    ...
    begin
      afile = File.open("app/assets/files/plan_mst.ace","r")
      if afile
        ...
        afile.each { |line|
          ...
          if line == '<<eof>>'
            ...
            @result = 1
            insert_plan_master(@final_split)
            if (@result == 0)
  ...
  def insert_plan_master(insertdata)
    begin
      lc = LogcreatorController.new
      pm = PlanMst.new
	  select_sql = "select * from plan_msts where plan_code = "+insertdata[0] 
	  @data = ActiveRecord::Base.connection.select_all(select_sql)
	  if(@data.length > 0)
		puts "Inside Delete loop"
		delete_sql = "Delete from plan_msts where plan_code = "+insertdata[0]
		@del_data = ActiveRecord::Base.connection.delete(delete_sql)
	  end
      pm.plan_code = insertdata[0]
      pm.plan = insertdata[1]
      pm.upd_flag = insertdata[2]
      pm.save
      @result = 0
    rescue =>err
      puts  err.message
