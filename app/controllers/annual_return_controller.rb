class AnnualReturnController < ApplicationController
  def read_annual_return
    Thread.new do
			annual_return()
		end
	...
	def annual_return
		...
		begin
		  afile = File.open("app/assets/files/mf_ans_return.ace","r")
		  if afile
			...
			afile.each { |line|
			  ...
			  if line == '<<eof>>'
				...
				@result = 1
				insert_annual_return(@final_split)
				if (@result == 0)
  ...
  def insert_annual_return(insertdata)

    begin

      lc = LogcreatorController.new
      mfar = MfAnsReturn.new
