class OptionMastController < ApplicationController

  def read_option_mast
    Thread.new do
			option_master()
		end
  ...
  def option_master
    ...
    begin
      afile = File.open("app/assets/files/option_mst.ace","r")
      if afile
        ...
        afile.each { |line|
          ...
          if line == '<<eof>>'
            ...
            @result = 1
            insert_option_mast(@final_split)
            if (@result == 0)
  ...
  def insert_option_mast(insertdata)

    begin

      lc = LogcreatorController.new
      om = OptionsMast.new
	  select_sql = "select * from options_masts where opt_code = "+insertdata[0] 
	  @data = ActiveRecord::Base.connection.select_all(select_sql)
	  if(@data.length > 0)
		puts "Inside Delete loop"
		delete_sql = "Delete from options_masts where opt_code = "+insertdata[0]
		@del_data = ActiveRecord::Base.connection.delete(delete_sql)
	  end
      om.opt_code = insertdata[0]
      om.option = insertdata[1]
      om.upd_flag = insertdata[2]
      om.save
      @result = 0
    rescue =>err
      puts  err.message
