class ThemeMastrController < ApplicationController
  ...
  def theme_master
    ...
    begin
      afile = File.open("app/assets/files/theme_mst.ace","r")
      if afile
  ...
  def insert_theme_mast(insertdata)
    begin
      lc = LogcreatorController.new
      thm = ThemeMst.new
	  select_sql = "select * from theme_msts where theme_code = "+insertdata[0] 
	  @data = ActiveRecord::Base.connection.select_all(select_sql)
	  if(@data.length > 0)
		puts "Inside Delete loop"
		delete_sql = "Delete from theme_msts where theme_code = "+insertdata[0]
		@del_data = ActiveRecord::Base.connection.delete(delete_sql)
	  end
      thm.theme_code = insertdata[0]
      thm.theme_type = insertdata[1]
      thm.upd_flag = insertdata[2]
      thm.save
      @result = 0
    rescue =>err
      puts  err.message
